...
import {
  NAVIGATION_BUTTON_DEFAULT_STYLES,
  STAGE_WIDTH,
  STAGE_HEIGHT,
  COLORS,
} from "../../constants";
 ...
 */
export function createKonvaButton(
  config: NavButton,
  onClick: (buttonId: string) => void,
): Konva.Group {
  // Calculate button dimensions
  const buttonWidth =
    config.style?.width || NAVIGATION_BUTTON_DEFAULT_STYLES.width;
  const buttonHeight =
    config.style?.height || NAVIGATION_BUTTON_DEFAULT_STYLES.height;
  ...
  const strokeColor =
    config.style?.stroke || NAVIGATION_BUTTON_DEFAULT_STYLES.stroke;
  const textFill =
    config.style?.textFill || NAVIGATION_BUTTON_DEFAULT_STYLES.textFill;
  ...
  buttonGroup.on("mouseenter", () => {
    if (buttonGroup.getAttr("disabled") || buttonGroup.getAttr("locked"))
      return;
